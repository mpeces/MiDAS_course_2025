# MiDAS workshop 2025 MEWE, Atlanta
#### Application of the MiDAS reference databases and ampvis2 for microbial community analyses


## Content
This repository contains data, exercises, and presentations for the hands on part of the MiDAS workshop.
For the exercises we will be using the [ampvis2](https://kasperskytte.github.io/ampvis2/articles/ampvis2.html) package in Rstudio.





## Relevant literature
- [MiDAS 4 paper](https://www.nature.com/articles/s41467-022-29438-7): A global catalogue of full-length 16S rRNA gene sequences and taxonomy for studies of bacterial communities in wastewater treatment plants.
- [MiDAS 5 paper](https://www.nature.com/articles/s41467-024-49641-y): MiDAS 5: Global diversity of bacteria and archaea in anaerobic digesters


## Contact
Sofie Zacho Vestergaard: szv@bio.aau.dk

Miriam Peces: mpg@bio.aau.dk
